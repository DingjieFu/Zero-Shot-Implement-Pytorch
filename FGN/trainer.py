...
from models import Generator, Discriminator, MLPClassifier
...
class Trainer:
    def __init__(
        ...
        self.optim_final_cls = optim.Adam(
            self.final_classifier.parameters(), lr=1e-4)
    ...
    def fit_classifier(self, img_features, label_attr, label_idx):
        ...
        Args:
            img         : batch_size X feature_size
            label_attr  : batch_size X attribute_size
    ...
    def get_gradient_penalty(self, X_real, X_gen):
        ...
        X_penalty = autograd.Variable(
            X_penalty, requires_grad=True).to(self.device)
    ...
    def fit_GAN(self, img_features, label_attr, label_idx, use_cls_loss=True):
        ...
        if use_cls_loss:
            ...
            log_prob = torch.log(torch.gather(
                Y_pred, 1, label_idx.unsqueeze(1)))
    ...
    def save_model(self, model=None):
        if "classifier" in model:
            pth_path = os.path.join(self.model_save_dir, model + ".pth")
            torch.save(self.classifier.state_dict(), pth_path)

        elif "gan" in model:
            dset_name = model.split('_')[0]
            g_pth_path = os.path.join(
                self.model_save_dir, "%s_generator.pth" % dset_name)
            torch.save(self.net_G.state_dict(), g_pth_path)

            d_pth_path = os.path.join(
                self.model_save_dir, "%s_discriminator.pth" % dset_name)
            torch.save(self.net_D.state_dict(), d_pth_path)

        elif "final_classifier" in model:
            pth_path = os.path.join(self.model_save_dir, model + ".pth")
            torch.save(self.final_classifier.state_dict(), pth_path)
    ...
    def load_model(self, model=None):
        if "classifier" in model:
            ...
            if os.path.exists(pth_path):
                self.classifier.load_state_dict(torch.load(pth_path))
                return True
            ...
            g_pth_path = os.path.join(
                self.model_save_dir, "%s_generator.pth" % dset_name)
            if os.path.exists(g_pth_path):
                self.net_G.load_state_dict(torch.load(g_pth_path))
                f1 = True

            d_pth_path = os.path.join(
                self.model_save_dir, "%s_discriminator.pth" % dset_name)
            if os.path.exists(d_pth_path):
                self.net_D.load_state_dict(torch.load(d_pth_path))
                f2 = True
            return f1 and f2
            ...
            pth_path = os.path.join(self.model_save_dir, model + ".pth")
            if os.path.exists(pth_path):
                self.final_classifier.load_state_dict(torch.load(pth_path))
                return True
        else:
