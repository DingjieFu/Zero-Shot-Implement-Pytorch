...
def zsl_acc(S_pred, S_gt, args):
    """
        ...
        - d -> feature dimension, N -> the number of features
    """
    dist = 1 - distCosine(S_pred.transpose(),
                          normalizeFeature(S_gt.transpose()))  # (N x N)
    # (N x HITK) 在前k个结果中 这里只计算top-1
    y_hit_k = np.zeros((dist.shape[0], args.HITK))

    for idx in range(0, dist.shape[0]):
        sorted_id = sorted(
            range(len(dist[idx, :])), key=lambda k: dist[idx, :][k], reverse=True)
...
if __name__ == '__main__':
    ...
    att_splits = scipy.io.loadmat(
        args.dataset_path + args.dataset + '/att_splits.mat')
    ...
    trainval_data = Features[:, np.squeeze(
        att_splits[trainval_loc]-1)]  # (d x N)
    ...
    semantic_predicted = np.dot(
        test_data.transpose(), normalizeFeature(W).transpose())  # (N x K)
    zsl_accuracy, _ = zsl_acc(semantic_predicted.transpose(), test_attr, args)
    print(
        f'[1] zsl accuracy for {args.dataset} dataset [F >>> S]: {zsl_accuracy:.2f}%')
    # [S --> F], projecting from semantic to visual space
    test_predicted = np.dot(normalizeFeature(
        test_attr).transpose(), normalizeFeature(W))  # (N x d)
    zsl_accuracy, _ = zsl_acc(test_predicted.transpose(), test_data, args)
    print(
        f'[2] zsl accuracy for {args.dataset} dataset [S >>> F]: {zsl_accuracy:.2f}%')
